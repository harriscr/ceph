...
class CBT(Task):
    ...
    def install_dependencies(self):
        ...
        if system_type == "rpm":
            ...
            cbt_depends = ["python3-pip", "librbd-devel", "pdsh", "pdsh-rcmd-ssh", "perf"]
        ...
        if benchmark_type in ["librbdfio", "fio"]:
            ...
            self.first_mon.run(
                args=["cd", os.path.join(testdir, "fio"), run.Raw("&&"), "./configure", run.Raw("&&"), "make"]
            )
    ...
    def checkout_cbt(self):
        ...
        if sha1 is None:
            self.first_mon.run(
                args=["git", "clone", "--depth", "1", "-b", branch, repo, "{tdir}/cbt".format(tdir=testdir)]
            )
        else:
            self.first_mon.run(args=["git", "clone", "-b", branch, repo, "{tdir}/cbt".format(tdir=testdir)])
            self.first_mon.run(
                args=[
                    "cd",
                    os.path.join(testdir, "cbt"),
                    run.Raw("&&"),
                    "git",
                    "reset",
                    "--hard",
    ...
    def setup(self):
        ...
        self.first_mon.write_file(
            os.path.join(self.cbt_dir, "cbt_config.yaml"), yaml.safe_dump(self.cbt_config, default_flow_style=False)
        )
        self.checkout_cbt()
